...
// Definisikan tipe data untuk Obat (Medicine)
interface Medicine {
  id: string;
  code: string;
  name: string;
...
export default function ObatPage() {
  ...
  const fetchData = useCallback(async (page = 1, search = '', type = '', status = '') => {
    setLoading(true);
    try {
      ...
      if (obatResult.success) {
        ...
        setCurrentPage(obatResult.pagination.page);
      } else {
  ...
  // PERBAIKAN: Buat fungsi pencarian yang di-debounce
  const debouncedSearch = useCallback(
    debounce((term: string) => {
      fetchData(1, term, filterType, filterStatus);
    }, 500), // Tunggu 500ms setelah user berhenti mengetik
    [filterType, filterStatus, fetchData]
  );
  ...
  // Efek untuk memuat data awal dan saat filter berubah
  useEffect(() => {
    const userData = localStorage.getItem('user');
    if (userData) setUser(JSON.parse(userData));
    fetchData(currentPage, searchTerm, filterType, filterStatus);
  }, [currentPage, filterType, filterStatus]); // Hapus 'searchTerm' dari sini
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (result.success) {
        ...
        fetchData(currentPage, searchTerm, filterType, filterStatus); // Muat ulang data
      } else {
  ...
  const getObatStatus = (obat: Medicine) => {
    ...
    if (expiry < today) return <Badge variant="destructive" className="bg-red-800 text-white">Kadaluwarsa</Badge>;
    if (obat.stock === 0) return <Badge variant="destructive">Habis</Badge>;
    if (expiry <= thirtyDaysFromNow) return <Badge variant="secondary" className="bg-yellow-100 text-yellow-800">Hampir ED</Badge>;
    if (obat.stock <= obat.minStock) return <Badge variant="secondary" className="bg-orange-100 text-orange-800">Menipis</Badge>;
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="relative">
                  ...
                  {/* PERBAIKAN: Gunakan handler baru */}
                  <Input placeholder="Cari nama atau kode obat..." value={searchTerm} onChange={handleSearchChange} className="pl-10" />
                </div>
                <Select value={filterType} onValueChange={setFilterType}><SelectTrigger><SelectValue placeholder="Semua Jenis" /></SelectTrigger><SelectContent><SelectItem value="Tablet">Tablet</SelectItem><SelectItem value="Kapsul">Kapsul</SelectItem><SelectItem value="Sirup">Sirup</SelectItem><SelectItem value="Cair">Cair</SelectItem></SelectContent></Select>
                <Select value={filterStatus} onValueChange={setFilterStatus}><SelectTrigger><SelectValue placeholder="Semua Status" /></SelectTrigger><SelectContent><SelectItem value="Tersedia">Tersedia</SelectItem><SelectItem value="Menipis">Menipis</SelectItem><SelectItem value="Habis">Habis</SelectItem><SelectItem value="Hampir ED">Hampir ED</SelectItem><SelectItem value="Kadaluwarsa">Kadaluwarsa</SelectItem></SelectContent></Select>
                      ...
                      <TableBody>
                        {obatList.map((obat) => (
                          <TableRow key={obat.id}>
                            <TableCell className="font-medium"><div>{obat.name}</div><div className="text-sm text-gray-500">{obat.code}</div>{obat.description && <div className="text-xs text-gray-400 mt-1 italic">{obat.description}</div>}</TableCell>
                            ...
                            <TableCell>{new Date(obat.expiryDate).toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric' })}</TableCell>
                            <TableCell>{getObatStatus(obat)}</TableCell>
                            <TableCell><div className="flex gap-2"><Link href={`/obat/${obat.id}/edit`}><Button variant="outline" size="icon"><Edit className="h-4 w-4" /></Button></Link>{user.role === 'ADMIN' && (<AlertDialog><AlertDialogTrigger asChild><Button variant="destructive" size="icon"><Trash2 className="h-4 w-4" /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Anda yakin?</AlertDialogTitle><AlertDialogDescription>Tindakan ini akan menghapus data obat secara permanen.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Batal</AlertDialogCancel><AlertDialogAction onClick={() => handleDelete(obat.id)}>Ya, Hapus</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>)}</div></TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
